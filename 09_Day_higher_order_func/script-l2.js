...
const filterPrices = function (price) {
  return typeof price == "number";
};

const sumPrices = function (acc, current) {
  return acc + current;
};
...
const sumPricesOnlyReduce = products.reduce((a, b) => {
...
console.log(sumPricesOnlyReduce);

const mostSpokenLanguages = (arr, numOfCountries) => {
  let info = {};
  arr.forEach((country) => {
    country.languages.forEach((language) => {
      console.log(language);
      info[language] = (info[language] || 0) + 1;
    });
  });
  console.log(Object.keys(info));
  const resultArr = Object.keys(info).map((language) => {
    return {
      Language: language,
      Count: info[language],
    };
  ...
  return resultArr
    ...
    .splice(0, numOfCountries);
};

console.log(mostSpokenLanguages(countries, 5));

const mostPopCountries = (arr, numOfCountries) => {
  let result = [];
  arr.map((country) => {
    result.push({
  ...
  return result.sort(sorted).slice(0, numOfCountries);
};

// console.log(mostPopCountries(countries, 10));

const toJadenCase = function (str) {
  let res = "";
  const strCopy = str.split(" ");
  strCopy.forEach((word) => {
...
// console.log(toJadenCase("How can mirrors be real"));

const mostUsedLetter = (letter) => {
  let res = {};
  countries.forEach((country) => {
    let firstLetter = country.name.charAt(0);
    if (!res[firstLetter]) {
      res[firstLetter] = [];
    }
    res[firstLetter].push(country.name);
  });
  return res[letter];
};

// console.log(mostUsedLetter("T"));
